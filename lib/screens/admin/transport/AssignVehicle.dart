...
class _AssignVehicleState extends State<AssignVehicle> {
      ...
      child: DropdownButton(
        ...
        items: routes.map((item) {
          return DropdownMenuItem<String>(
            ...
            child: Padding(
              ...
              child: Text(item.title,style: Theme.of(context).textTheme.headline4,),
            ),
          );
        }).toList(),
        ...
        onChanged: (value) {
          setState(() {
            ...
            debugPrint('User select $selectedRouteId');
          });
        },
        value: selectedRoute,
      ),
      ...
      child: DropdownButton(
        ...
        items: vehicles.map((item) {
          return DropdownMenuItem<String>(
            ...
            child: Padding(
              padding: const EdgeInsets.only(left: 8.0, bottom: 10.0),
              child: Text(item.vehicleNo,style: Theme.of(context).textTheme.headline4,),
            ),
          );
        }).toList(),
        style: Theme.of(context).textTheme.headline4.copyWith(fontSize: 13.0),
        onChanged: (value) {
          setState(() {
            ...
            print('SELECTED Route: $selectedVehicle');
            debugPrint('User select $selectedVehicleId');
          });
        },
  ...
  assignVehicle() async {
    ...
    if (response.statusCode == 200) {
      Utils.showToast('$selectedVehicle assigned to $selectedRoute');
      setState(() {
        isResponse = false;
      });
      return true;
      ...
      if (response.data['success'] == false)
        Utils.showToast(
            '$selectedVehicle is already assigned to $selectedRoute');
      return true;
    } else {
      return false;
    }
